// src/components/pages/NovelDetails.jsx (ФИНАЛЬНАЯ ПОЛНАЯ ВЕРСИЯ)

import React, { useState, useMemo, useEffect, useRef } from 'react';
import { supabase } from "../../supabase-config";
import { LockIcon } from '../icons.jsx'; // Предполагаем, что иконки на месте
...
export const NovelDetails = ({ novel, onSelectChapter, onGenreSelect, subscription, botUsername, userId, chapters, isLoadingChapters, lastReadData, onBack }) => {
    ...
    // Обновление счетчика просмотров через Supabase
    useEffect(() => {
        if (user && novel?.id) {
    ...
    // --- ВСЕ ВАШИ ОРИГИНАЛЬНЫЕ СОСТОЯНИЯ И ЛОГИКА ---
    ...
    const sortedChapters = useMemo(() => {
        ...
        // ВАЖНО: .reverse() мутирует массив, поэтому работаем с копией
        if (sortOrder === 'newest') return chaptersCopy.sort((a, b) => b.id - a.id);
        return chaptersCopy.sort((a, b) => a.id - a.id);
    }, [chapters, sortOrder]);
    ...
    const handlePaymentMethodSelect = async (method) => {
        ...
        if (!tg || !userId || !selectedPlan) { return; }
        tg.showConfirm(
    ...
    return (
        <div className="text-text-main">
            ...
            <div className="relative h-64">
                <img src={`/${novel.cover_url}`} alt={novel.title} className="w-full h-full object-cover object-top absolute"/>
            ...
            <div className="p-4">
                <div className="flex flex-wrap gap-2 mb-4">
                    {novelGenres.map(genre => {
                        ...
                        const genreClassName = `text-xs font-semibold px-3 py-1 rounded-full transition-colors duration-200 bg-component-bg border hover:bg-border-color ${isHighlighted ? 'border-genre-highlight-border text-genre-highlight-text' : 'border-border-color text-text-main'}`;
                ...
                <div ref={descriptionRef} className={`relative overflow-hidden transition-all duration-500 ${isDescriptionExpanded ? 'max-h-full' : 'max-h-24'}`}>
                    <div className="text-sm mb-2 opacity-80 font-body prose dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: novel.description }} />
                </div>
                {isLongDescription && <div className="text-right mt-2"><button onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)} className="text-sm font-semibold text-accent mb-4">{isDescriptionExpanded ? 'Скрыть' : 'Читать полностью...'}</button></div>}
                ...
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setSortOrder(prev => prev === 'newest' ? 'oldest' : 'newest')} className="text-sm font-semibold text-accent">
                        {sortOrder === 'newest' ? 'Сначала новые' : 'Сначала старые'}
                    </button>
                </div>
                
                {isLoadingChapters ? <p>Загрузка глав...</p> : (
                    <div className="flex flex-col gap-3">
                        {sortedChapters.map((chapter, index) => {
                            ...
                            return (
                                <div key={chapter.id} onClick={() => handleChapterClick(chapter)} className={`p-4 bg-component-bg rounded-xl cursor-pointer transition-all duration-200 hover:border-accent-hover hover:bg-accent/10 border border-border-color flex items-center justify-between shadow-sm hover:shadow-md ${showLock ? 'opacity-70' : ''}`}>
                                    ...
                                    <div className="text-right flex-shrink-0 ml-4">
                                        ...
                                        <p className="text-text-main opacity-70 text-xs mt-1">{formatDate(chapter.published_at)}</p>
                                    </div>
                                </div>
                            );
                        })}
